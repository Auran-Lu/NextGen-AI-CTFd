...
export class TargetCTFPanel extends Panel {
  ...
  // 靶标被攻击之后的状态提示（修复重复霸榜问题）
  addItem(name: string) {
    const count = this.listEl.childElementCount
    if (count >= 3) return

    // 遍历查找去重
    const existingNames = Array.from(this.listEl.children).map(child => {
      const tnameSpan = child.querySelector('.tname')
      return tnameSpan ? tnameSpan.innerHTML : ''
    })
